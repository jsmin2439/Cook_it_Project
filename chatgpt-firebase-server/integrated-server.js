...
// Firebase에서 레시피를 가져오고 사용자의 식재료와 매칭
  ...
  try {
    ...
    const gptResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "상위 50개 레시피 중에서 재료 매칭도와 카테고리 다양성을 고려하여 최적의 3개 레시피를 추천하세요."
        },
        {
          role: "user",
          content: `
...
다음 형식으로 응답해주세요:
{
  "recommendedRecipes": ["레시피ID1", "레시피ID2", "레시피ID3"]
    ...
    // 백업: 추천 결과가 없거나 3개 미만인 경우
    if (!recommendedRecipes || recommendedRecipes.length < 3) {
      console.log("Insufficient GPT recommendations, using fallback logic");
    ...
    if (error.code === 'rate_limit_exceeded') {
      console.log("Rate limit exceeded, using fallback logic");
      return topRecipes.slice(0, 3).map(recipe => recipe.fullData);
    }
    throw error;
  }
...
app.post("/upload-ingredient", upload.single("image"), async (req, res) => {
  try {
    ...
    const topObject = objects
      .filter(o => o.score > 0.7 && o.name !== "Food")
    ...
    await saveIngredients(userId, [topObject.name]);

    res.json({ success: true, detectedIngredient: topObject.name });
  } catch (error) {
    ...
    res.json({
      ...
      recommendedRecipes: recommendedRecipes.map(recipe => ({
        ...
        cookingSteps: {
          ...
          step6: recipe.MANUAL06
        },
        images: {
          main: recipe.ATT_FILE_NO_MAIN,
          steps: {
            step1: recipe.MANUAL_IMG01,
            step2: recipe.MANUAL_IMG02,
            step3: recipe.MANUAL_IMG03,
            step4: recipe.MANUAL_IMG04,
            step5: recipe.MANUAL_IMG05,
            step6: recipe.MANUAL_IMG06
          }
        },
        matchScore: recipe.matchScore,
